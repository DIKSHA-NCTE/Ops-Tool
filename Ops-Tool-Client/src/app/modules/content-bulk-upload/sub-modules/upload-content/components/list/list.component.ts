...
export class ListComponent implements OnInit {
  ...
  constructor(
    ...
    private _contentUpload: ContentsBulkUploadService,
  ...
  ngOnInit(): void {
    ...
    this.getSearchForm();
    this.payload = {};
    this.contentUploadForm = this._fb.group({
  ...
  getSearchForm() {
    this._contentUpload.getContentUploadFilter().subscribe((resp: any) => {
  ...
  onChangeHandler = (val: any) => {
    if (val.dataType == 'array' && val.value.length > 0) {
      ...
      if(val.property == 'id' && val.value.length > 0) {
        ...
        this._contentUpload.channelRead(req).subscribe((result: any) => {
      ...
      this.contentUploadForm.patchValue({ [val.property]: val.value });
    } else if (
      val.dataType == 'string' &&
      val.value != '' &&
      val.value != null
      ...
      this.contentUploadForm.patchValue({ [val.property]: val.value.trim() });
    } else {
      delete this.payload[`${val.property}`];
      this.contentUploadForm.patchValue({ [val.property]: '' });
    }
  };
  ...
  uploadContent(formData: any) {
    this._contentUpload.uploadContent(formData).subscribe((result: any) => {
  ...
  clearFields(key: any) {
    ...
    this.contentUploadForm.patchValue({ [key.key]: '' });
