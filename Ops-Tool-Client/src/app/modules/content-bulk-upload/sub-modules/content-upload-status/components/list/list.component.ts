...
export class ListComponent implements OnInit {
  tenantsList: any = [];
  contentUploadStatusForm!: FormGroup;
  batch_processId: any;
  ...
  constructor(
    ...
    private _contentUploadStatus: ContentsBulkUploadService,
  ...
  ngOnInit(): void {
    ...
    this.getSearchForm();
    this.payload = {};
    this.contentUploadStatusForm = this._fb.group({
      channel: [''],
      userInfo: '',
      processId: '',
      contentStatus: ['']
  ...
  getSearchForm() {
    this._contentUploadStatus.getContentUploadStatusFilter().subscribe((resp: any) => {
  ...
  onChangeHandler = (val: any) => {
    if (val.dataType == 'array' && val.value.length > 0) {
      ...
      this.contentUploadStatusForm.patchValue({
      ...
      this.contentUploadStatusForm.patchValue({ [val.property]: val.value });
    } else if (
      val.dataType == 'string' &&
      val.value != '' &&
      val.value != null
      ...
      this.contentUploadStatusForm.patchValue({ [val.property]: val.value.trim() });
    } else {
      delete this.payload[`${val.property}`];
      this.contentUploadStatusForm.patchValue({ [val.property]: '' });
    }
  };
  ...
  contentUploadStatus() {
    ...
    let request = this.payload
      this._contentUploadStatus.getContentUploadStatus(request).subscribe((result: any) => {
  ...
  resetForm() {
    this.payload = {};
    this.contentUploadStatusForm.reset();
  ...
  clearFields(key: any) {
    ...
    this.contentUploadStatusForm.patchValue({ [key.key]: '' });
