...
let PERMISSIONS_HELPER = {
  ...
  getCurrentUserRoles: function (reqObj, callback) {
    ...
    var options = {
      method: 'GET',
      url: config.BASE + apiConfig.SUB_URL.API + apiConfig.SUB_URL.USER + apiConfig.SUB_URL.V1 + apiConfig.APIS.READ + '/' + userId,
      headers: {
        'x-msgid': uuidv1(),
        'ts': dateFormat(new Date(), 'yyyy-mm-dd HH:MM:ss:lo'),
        'content-type': 'application/json',
    ...
    const telemetryData = {reqObj: reqObj,
      options: options,
      uri: config.BASE + apiConfig.SUB_URL.API + apiConfig.SUB_URL.USER + apiConfig.SUB_URL.V1 + apiConfig.APIS.READ,
      type: 'user',
      id: userId,
      userId: userId}
    // telemetryHelper.logAPICallEvent(telemetryData)
